...
func (cm *ControlMapper) LoadGangs() ([]*GangedFader, error) {
	...
	for i, gangControl := range cm.config.GangControls {
		...
		gang, err := NewGangedFader(gangControl.Name, gangControl.Unit, GangModeMirror, gangChannels, levelControls, gangControl.TaperDb, gangControl.ShowVUMeter, gangControl.ShowTrackColor, cm.config.LevelSmoothing)
		if err != nil {
